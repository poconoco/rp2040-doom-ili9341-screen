...
- RP2040-Plus (the RP2040 board with Type-C connector and 16Mb or flash storage).
  This code also should be compatible with standard Pi Pico with 2Mb of storage, but you'll have to use the super-tiny mode (see below), and you will be limited to small shareware doom wads. Full DOOM2 requires ~8 Mb of space, so RP2040-Plus is a perfect fit.
...
- Li-ion battery
- Switch putton to turn power on/off
- RP2040-Plus has onboard charging and power control circuits, so no additional modules needed for this. Bare PiPico may require charger module and/or something else.
...
The Doom2 wad requires Pi Pico with 16 Mb of flash (compressed file is 7.4 Mb, so may also fit on 8Mb, but there should also be place for the code, and I don't have such a board to test on). Also it requires the `-no-super-tiny` flag passed to `whd_gen`. If you are running on a standard 2Mb board, you can only use doom1 shareware wad, and DO NOT pass wthe `-no-super-tiny` flag.
...
Hold the BOOTSEL button on your Pico and plug it into your USB port. Then flash the whd file we generated earlier using picotool we built earlier:
```
cd ~/rp2040-doom-ili9341-screen
...
Now, when pico is still connected and mounted as a flash drive, copy to the root of that drive the `src/doom_tiny_nost.uf2` or `src/doom_tiny.uf2` (depending on if you are using super-tiny whd or not, see above) file.

When finish copying, Pico should reboot automatically. And if everyting was wired, built and flashed correctly, DOOM should run.
