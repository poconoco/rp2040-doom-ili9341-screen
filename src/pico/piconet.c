...
void piconet_init() {
    ...
    uint periodic_alarm_irq = TIMER_ALARM_IRQ_NUM(timer_hw, PERIODIC_ALARM_NUM);
    irq_set_priority(periodic_alarm_irq, 0xc0); // don't want timer pre-empting the other IRQs
    irq_set_enabled(periodic_alarm_irq, true); // no harm turning it on
}
